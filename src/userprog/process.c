...
>>>>>>> 3c6c1dc (Add proj2 folder and its files)
  ...
  while (file_name[len] != '\0' && file_name[len] != ' ') {
      cmd_file[len] = file_name[len];
      len++;
  }
  cmd_file[len] = '\0';

  struct file *file = filesys_open(cmd_file);
  if (file == NULL) {
...
int process_wait(tid_t child_tid) 
{
    ...
    for (elem = list_begin(&(curr->child_list)); elem != list_end(&(curr->child_list)); elem = list_next(elem)) {
        struct thread *child_cand = list_entry(elem, struct thread, child_elem);

        if (child_cand->tid == child_tid) {
            child = child_cand;
            break;
        }
    }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  strlcpy(copy_file_name, file_name, sizeof(copy_file_name));
  token = strtok_r(copy_file_name, " ", &next);

  
  while (token != NULL) {
    argv[argc] = token;  // Store each argument in argv
    argc++;  // Increment argument count
    token = strtok_r(NULL, " ", &next);  // Get the next token
  }
  argv[argc] = NULL;
...
void make_stack(char* argv[], void **esp, int argc) {
    uintptr_t argv_addresses[128];
    
    for (int i = argc - 1; i >= 0; i--) {
        size_t arg_length = strlen(argv[i]) + 1;
        *esp -= arg_length;
        memcpy(*esp, argv[i], arg_length);
        argv_addresses[i] = (uintptr_t)*esp;
    }

    size_t padding_size = (uintptr_t)*esp % 4;
    if (padding_size > 0) {
        *esp -= padding_size;
    ...
    *esp -= sizeof(uintptr_t);
    ...
    for (int i = argc - 1; i >= 0; i--) {
        *esp -= sizeof(uintptr_t);
        memcpy(*esp, &argv_addresses[i], sizeof(uintptr_t));
    }

    uintptr_t argv_addr = (uintptr_t)*esp;
    *esp -= sizeof(uintptr_t);
    memcpy(*esp, &argv_addr, sizeof(uintptr_t));
